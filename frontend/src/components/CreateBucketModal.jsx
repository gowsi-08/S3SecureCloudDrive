...
import { X, Cloud, Copy, Check, AlertCircle, Eye, EyeOff, ChevronRight } from 'lucide-react';
import toast from 'react-hot-toast';
import { cloudConfigAPI } from '../services/api';

const CreateBucketModal = ({ isOpen, onClose, onSuccess }) => {
  const [step, setStep] = useState(1); // 1: Instructions, 2: Credentials
  ...
  const handleConnect = async () => {
    try {
      ...
      toast.loading('🔄 Connecting to bucket...', { id: 'connect' });
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-6 sticky top-0">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="ml-auto p-2 text-white hover:bg-white hover:bg-opacity-20 rounded-lg transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
